...
/**
 * 调用MSTTS将字符串转换成语音信息 调用windows speech API(SAPI)
 * 
 * @author suyunlong
 * 
 */
public class MSTTSSpeech {
	...
	private HashMap<Integer, String> voiceMapping;
	...
	public MSTTSSpeech() {
		voiceMapping = new HashMap<Integer, String>();
		ComThread.InitSTA();
		if (ax == null) {
	...
	public String[] getVoices() {
		...
		System.out.println("可用语音：" + voiceMapping);
		return voices;
	}
	...
	public static void main(String[] args) {
		...
		// 1=Microsoft Mike,
		// 2=Microsoft Sam,
		// 3=Microsoft Simplified Chinese,
		// 4=Sample TTS Voice
		// speech.getVoices();
		speech.changeVoice(3);

		String text = "我的测试,物理内存至少需要512MB，建议2GB以上,虚拟内存是主机物理内存的两倍，不要设到系统盘,硬盘空闲空间大于4.77GB.";

		//text = "<lang langid='409'>Hello Word</lang> <lang langid='804'>对象浏览器 不能用45</lang>";

		speech.speak(text);
		speech.setFormatType(6); // speech.setRate(-1);
		speech.saveToWav(text, "d://test.wav");
		SoundUtilForMSTTSSpeech.wavToPcm("d://test.wav");
